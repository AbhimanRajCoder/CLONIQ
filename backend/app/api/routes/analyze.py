...
def _store_and_return(result: Dict[str, Any]) -> Dict[str, Any]:
...
async def analyze(files: List[UploadFile] = File(...)):
    if len(files) < 2:
        raise HTTPException(
            status_code=400,
            detail="Please upload at least 2 Python files to compare.",
        )
    ...
    for upload in files:
        ...
        if not filename.endswith(".py"):
            errors.append({"file": filename, "error": "Not a .py file – skipped."})
            continue
...
async def analyze_pair(
    ...
    for upload in [file1, file2]:
        filename = upload.filename or "unknown.py"
        if not filename.endswith(".py"):
            errors.append({"file": filename, "error": "Not a .py file."})
            continue
...
async def analyze_advanced(
    ...
    for upload, filename in uploads:
        if not filename.endswith(".py"):
            raise HTTPException(
                status_code=400,
                detail=f"'{filename}' is not a .py file. Only Python files are accepted.",
            )
...
async def compare_github_repos(body: GitHubCompareRequest):
    errors: List[Dict[str, str]] = []
...
async def analyze_google_sheet(body: GoogleSheetRequest):
    """
