...
github_service.py
─────────────────
Fetches Python source files from public GitHub repositories
using the GitHub REST API. Works entirely in-memory.
...
def get_py_file_paths(owner: str, repo: str, branch: str) -> List[Dict[str, Any]]:
    """
    Fetch the full file tree for a repo and return metadata for .py files only.

    Returns a list of dicts with keys: path, size, sha, url
...
def fetch_repo_py_files(repo_url: str) -> Dict[str, str]:
    ...
    filename → source code for all Python files in the repository.

    This is the main entry point for the GitHub service.
    """
    owner, repo = parse_repo_url(repo_url)
    branch = get_default_branch(owner, repo)
    file_metas = get_py_file_paths(owner, repo, branch)

    if not file_metas:
        raise ValueError(
            f"No Python files found in {owner}/{repo} (branch: {branch})."
        )
