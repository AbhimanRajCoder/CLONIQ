...
Two {language} programs have a STRUCTURAL similarity score of: {similarity_score}

Breakdown:
- AST similarity: {ast_score}
- Control Flow Graph similarity: {cfg_score}
- Data Dependency Graph similarity: {dfg_score}
{module_graph_line}{framework_line}
...
# Language-specific context injected into the prompt
_LANGUAGE_CONTEXTS = {
    "python": (
        "For Python: consider standard library patterns, PEP8 conventions, "
        "and common algorithm implementations that naturally look alike."
    ),
    "javascript": (
        "For JavaScript/React: consider that boilerplate hooks (useState, useEffect), "
        "standard component patterns, and Next.js conventions naturally produce "
        "high structural similarity. JSX layout patterns are especially common. "
        "Focus on unique business logic, custom hooks, and non-trivial state management."
    ),
    "typescript": (
        "For TypeScript/React: consider that boilerplate hooks (useState, useEffect), "
        "standard component patterns, Next.js app router conventions, and type "
        "definitions naturally produce high structural similarity. "
        "Focus on unique business logic, custom hooks, and non-trivial state management."
    ),
}
...
def evaluate_pair(
    ...
    module_line = ""
    if scores.module_similarity > 0:
        module_line = f"\n- Module Graph similarity: {scores.module_similarity:.4f}"

    framework_line = ""
    if scores.framework:
        framework_line = f"\n- Framework detected: {scores.framework}"

    lang_key = scores.language.lower()
    lang_context = _LANGUAGE_CONTEXTS.get(lang_key, "")
    ...
    prompt = _PROMPT_TEMPLATE.format(
        ...
        module_graph_line=module_line,
        framework_line=framework_line,
        language_specific_context=lang_context,
        code_1=code_a,
        code_2=code_b,
    )
