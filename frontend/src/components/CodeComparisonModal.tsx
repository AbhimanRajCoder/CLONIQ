...
export default function CodeComparisonModal({
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Code Comparison" maxWidth="max-w-7xl">
                        ...
                        <div className="mt-4 pt-4 border-t border-white/5">
                            <details className="group">
                                ...
                                <div className="mt-2 p-3 rounded-lg bg-black/20 text-xs text-slate-400 leading-relaxed font-mono">
                                    {refined.reasoning || "AI explanation is not available (API quota exceeded or model error)."}
                                </div>
                            </details>
                        </div>
                    </motion.div>
            ...
            {pair.matching_regions.filter(r => (r.file1_lines[1] - r.file1_lines[0] > 0) || (r.file2_lines[1] - r.file2_lines[0] > 0)).length === 0 ? (
                <p className="text-slate-400 text-center py-8 italic opacity-50 underline decoration-slate-800">No structural matching regions found (or only 1-line matches ignored).</p>
            ) : (
                <div className="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                    {pair.matching_regions
                        .filter(r => (r.file1_lines[1] - r.file1_lines[0] > 0) || (r.file2_lines[1] - r.file2_lines[0] > 0))
                        .slice(0, 20)
                        .map((region, idx) => (
                            <motion.div
                                key={idx}
